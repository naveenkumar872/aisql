        ...
        with st.container():
            if isinstance(output, pd.DataFrame) and not output.empty:
                ...
                with tabs[2]:
                    ...
                    num_rows = st.text_input("Enter number of rows", "All",key=f"{j}" )
                  
                    #ques = f"Visualize the data as a {ques} using {num_rows} rows."
                    ques = f"Create a {ques} using {'all rows' if num_rows.lower() == 'all' else f'the first {num_rows} rows'} of the dataset, highlighting key trends or patterns."
                    ...
                    if st.button("Visualize", key=f"visualize_button_{i}"):
